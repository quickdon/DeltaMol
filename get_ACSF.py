...
Rs = np.linspace(0.5, 4.0, 8)  # 定义距离范围
eta_r = np.geomspace(0.01, 0.1, 3)
g2_params = [[eta, Rs_i] for eta in eta_r for Rs_i in Rs]

lambda_vals = [-1, 1]
zeta_vals = [1, 2, 4, 8]
eta_a = [0.01, 0.05]
g4_params = [[eta, zeta, lam]
             for eta in eta_a
             for zeta in zeta_vals
             for lam in lambda_vals]

# acsf = generate_acsf(Z_list[0], coords_list[0], species)
acsf = ACSF(
    species=species,
    r_cut=4.0,  # 截断半径
    g2_params=g2_params,  # G2参数
    g4_params=g4_params,  # G4参数
)

soap_cache_file = "acsf_features_cache.h5"
num_mols = len(Z_list)
...
if not os.path.exists(soap_cache_file):
    ...
    with h5py.File(soap_cache_file, 'w') as f:
        ...
        for i, Z in enumerate(Z_list):
            ...
            soap_matrix = acsf.create(atoms, n_jobs=-1)
